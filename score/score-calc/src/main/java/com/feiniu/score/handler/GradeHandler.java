...
public class GradeHandler implements KafkaStreamHandler {

    private static final CustomLog LOGGER = CustomLog.getLogger(GradeHandler.class);
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    LOGGER.info("kafkaconsumer grade send message info: partition=" + msg.partition() + ",offset=" + msg.offset() + ", message=" + message);
                    memGuid = getMemGuid(message);
                    this.growthMemService.getkafkafromCRM(memGuid, message);
                    LOGGER.info("grade consumer success");
                } catch (Exception e) {
                    LOGGER.error("等级消息处理失败。message" + message, e);
                    try {
                        scoreExceptionHandler.handlerBizException(e, memGuid, message, Constant.GROWTH_COSUMER_GRADE_RECEIVE);
                    } catch (Exception e2) {
