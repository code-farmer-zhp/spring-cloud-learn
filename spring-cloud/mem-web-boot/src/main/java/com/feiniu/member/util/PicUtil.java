...
public class PicUtil {
    ...
    public static String picTransform(String picUrl, String storeUrl){
        ...
        if(split[split.length-2].indexOf("_60x60")<0){
          picUrl = picUrl.replace("."+split[split.length-1],"_80x80."+split[split.length-1]);
        }
    ...
    public static String picTransform(String picUrl, String storeUrl, String size, String type, String imgInsideUrl, Boolean touch) {
        if(StringUtils.isBlank(picUrl)){
            return picUrl;
        }
         picUrl = picUrl.replace("_400x400", "").replace("_80x80", "").replace("_200x200", "");
        if(type.equals("1")){
            if(StringUtils.isNotBlank(size)){
                ...
                picUrl = picUrl.replace(".jpg_60x60.jpg", "_60x60.jpg");
                picUrl = picUrl.replace("http://wh-image01.fn.com:80/", "https://imgsvr01.beta1.fn/");
                picUrl = picUrl.replace("http://wh-image01.fn.com/", "https://imgsvr01.beta1.fn/");
                picUrl = picUrl.replace("http://10.211.64.68", "https://imgsvr01.beta1.fn");
                picUrl = picUrl.replace("img10","img"+ Integer.toString(random.nextInt(3)+16));
                String[] split = picUrl.split("\\.");
                if(split[split.length-2].indexOf("_60x60")<0){
        ...
        if(touch){
            picUrl= picUrl.replace(".jpg", "_q75.jpg").replace(".png", "_q75.png").replace("http://", "https://");
        }
